...
namespace BartekNizio.Unity.Template.Entitas
{
	public class GameObjectPoolService
	{
		...
		private readonly Dictionary<GameObject, GameObjectPool> _instance2pool;

		private readonly Dictionary<GameObject, GameObjectPool> _prefab2pool;
		...
		private GameObject InternalCreate(GameObject prefab, Vector3 position, Quaternion rotation) {
			var pool = GetOrCreatePool(prefab);
			var obj = pool.Instantiate(position, rotation);
