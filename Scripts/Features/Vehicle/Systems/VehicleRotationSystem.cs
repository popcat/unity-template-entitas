	...
	public class VehicleRotationSystem : IExecuteSystem
	{
		...
		public void Execute() {
			foreach (var vehicleEntity in _vehicleGroup.GetEntities(_vehicleCache)) {
				if (vehicleEntity.hasVehicleLookRotation == false) {
					continue;
				}

				var rotationSpeed = vehicleEntity.vehicle.config.rotationSpeed * Time.fixedDeltaTime;
				vehicleEntity.vehicle.dirPivot.rotation = Quaternion.Slerp(vehicleEntity.vehicle.dirPivot.rotation,
					vehicleEntity.vehicleLookRotation.value, rotationSpeed);
			}
		}
	}
}