	...
	public class VehicleWobbleSystem : IExecuteSystem
	{
		...
		public void Execute() {
			foreach (var vehicleEntity in _vehicleGroup.GetEntities(_vehicleCache)) {
				if (vehicleEntity.hasVehicleMovementPower == false) {
					continue;
				}

				if (vehicleEntity.hasVehicleMovementDirection == false) {
					continue;
				}
				...
				vehicleEntity.vehicle.accPivot.localRotation =
					Quaternion.Slerp(vehicleEntity.vehicle.accPivot.localRotation, targetRot, rotationSpeed);
			}
		}
	}
}