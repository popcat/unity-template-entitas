...
namespace BartekNizio.Unity.Template.Entitas
{
	public class PriorityQueue<TElement, TPriority>
	{
		...
		/// <remarks>
		///     The method performs a linear-time scan of every element in the heap, removing the first value found to match the
		///     <paramref name="element" /> parameter.
		///     In case of duplicate entries, what entry does get removed is non-deterministic and does not take priority into
		///     account.
		///     If no <paramref name="equalityComparer" /> is specified, <see cref="EqualityComparer{TElement}.Default" /> will be
		///     used instead.
		/// </remarks>
		public bool Remove(
			TElement element,
		...
		private void Grow(int minCapacity) {
			...
			if ((uint)newcapacity > EnumerableHelpers.ArrayMaxLength) {
				newcapacity = EnumerableHelpers.ArrayMaxLength;
			}
